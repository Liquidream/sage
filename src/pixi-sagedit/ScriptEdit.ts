...
export class ScriptEdit {
  ...
  public waitSkippable(seconds: number): Promise<void> {
    const timedPromise = new Promise<void>((res) =>
      setTimeout(res, seconds * 1000)
    )
    return Promise.race([timedPromise, this.waitForSkip()])
  }

  public async safeExecFunc(strFunc: string) {
    try {
      // https://stackoverflow.com/questions/47945024/dynamically-create-async-function
      const AsyncFunction = Object.getPrototypeOf(
        async function () {}
      ).constructor
      const myAsync = new AsyncFunction(strFunc)
      await myAsync()
      // Old way (only works with .ts files)
      //Function(strFunc)()
    } catch (e: unknown) {
      if (e instanceof Error) {
        SAGEdit.Dialog.showErrorMessage(
          `Error running script function: ${e.message}`
        )
      }
    }
  }
  ...
  private waitForSkip(): Promise<void> {
    return new Promise<void>((res) => {
      function onSkip() {
        res()
        SAGEdit.Events.removeListener("sceneinteract", onSkip)
        //document.removeEventListener('keypress', onSkip);
      }
      SAGEdit.Events.on("sceneinteract", onSkip, this)
    })
  }
}
