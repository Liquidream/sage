...
export class Dialog {
  ...
  public async showChoices(
    ...
    if (this._dialogChoices) {
      ...
      if (options?.suppressChoiceSelectRepeat)
        this.suppressChoiceSelectRepeat = options.suppressChoiceSelectRepeat
    ...
    this.dialogContainer.pivot.set(
  ...
  private async showMessageCore(options: {
    ...
    if (this.dialogBackground) {
      ...
      // clear existing message
      console.debug("--->> clear message, another coming in...")
      this.clearMessage()
    }
    ...
    this.dialogBackground.pivot.set(
      ...
      this.dialogBackground.height / 2
    )
    ...
    if (options.durationInSecs) {
      ...
      waitDuration = clamp(
        options.message.length / this.CHARS_PER_SEC,
        this.MIN_DURATION_SEC,
    ...
    if (waitDuration > 0) {
      ...
      if (newDialogText === this.dialogText) {
        ...
        // Stop playing audio (if applicable)
        if (SAGE.Sound.soundLibrary.exists(this.dialogSoundName)) {
          const sound = SAGE.Sound.soundLibrary.find(this.dialogSoundName)
