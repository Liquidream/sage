...
export class Player implements IPlayerData, Serialization<Player> {
  ...
  public inventory: Array<PropData.IPropData> = [];
  ...
  public addToInventory(propData: PropData.IPropData) {
    SAGE.World.player.inventory.push(propData);
    SAGE.invScreen.addProp(new Prop(propData))
  }

  /** Remove (and return) the specified prop, if present */
  public removeFromInventory(propId: string): PropData.IPropData | undefined {
...
export interface IPlayerData {
  ...
  inventory: Array<PropData.IPropData>;
  // Key-Value pair to allow properties to be set/read
